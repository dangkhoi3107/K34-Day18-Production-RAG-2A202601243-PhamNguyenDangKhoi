...
class DenseSearch:
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        # recreate_collection() đã deprecated từ qdrant-client 1.11 →
        # delete + create để không dựa vào API cũ (repo đang chạy 1.19).
...
def reciprocal_rank_fusion(results_list: list[list[SearchResult]], k: int = 60,
    ...
    sorted_docs = sorted(rrf_scores.items(), key=lambda x: x[1]["score"], reverse=True)[:top_k]
    return [
        SearchResult(
            text=text,
            score=data["score"],
