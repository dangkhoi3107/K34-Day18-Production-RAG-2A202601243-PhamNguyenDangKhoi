...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            if content:
                ...
                qs = [q.strip().lstrip("0123456789.-) ") for q in questions if q.strip()]
                if qs:
    ...
    return ["Quy định cụ thể trong đoạn văn là gì?"]
...
def enrich_chunks(
    ...
    for i, chunk in enumerate(chunks):
        ...
        enriched.append(EnrichedChunk(
            ...
            # Metadata gốc đặt SAU nên luôn thắng: source/version/section là
            # bằng chứng để xử lý xung đột v2023 vs v2024, không được để
            # metadata LLM sinh ra ghi đè mất.
