...
- **Điều bất ngờ nhất — pipeline "xịn" hơn không tự động cho điểm cao hơn.** Lần chạy đầu tiên, production pipeline (hierarchical chunking + hybrid + rerank + enrichment) **thua naive baseline ở cả 4 metric**. Đây là bài học lớn nhất của lab: thêm component không phải là tối ưu; phải đo rồi mới biết component nào đang làm hỏng chất lượng. Chi tiết ở [`analysis/failure_analysis.md`](../failure_analysis.md).
